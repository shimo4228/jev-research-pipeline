...
source-side trouble: a missing key, an HTTP error status, a transport error or an
unparseable body becomes FetchOutcome.failure (counted in the operations section).
Only programming errors — and CassetteMiss in tests — propagate.
"""
...
class Draft(Value):
    """What a parser extracts from one result before it becomes a SourceItem."""
...
class Adapter:
    ...
    async def fetch(
        ...
        *,
        ...
        try:
            drafts = self.parse(response.text)
            sources = tuple(
                SourceItem.new(
                    line=line.id,
                    adapter=self.kind,
        ...
        return FetchOutcome(adapter=self.kind, query=query, sources=sources, failure=None)
